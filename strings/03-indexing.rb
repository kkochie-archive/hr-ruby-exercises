...
Ruby also allows you to index strings using a Range or Regexp object as well. 
...
You can use string interpolation to insert Ruby code within a string. For example:
  > tmp = 123
  > "Hello #{tmp}"
  Hello 123
...
# Find the avg of XX.XX and YY.YY and round off to two decimal places. Return with SSS-prefix.
# First attempt
def serial_average(str)
  ...
  yy = str[10, 5].to_f
  avg = (xx + yy) / 2
  return "#{sss}-#{avg.round(2)}"
end

# Clean it up
def serial_average(str)
  "#{str[0,3]}-#{((str[4,5].to_f + str[10,5].to_f) / 2 ).round(2)}"
end
