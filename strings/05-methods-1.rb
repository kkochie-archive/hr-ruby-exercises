...
• String.chomp(separator=$/) - returns a new string with the given separator removed from the end of the string (if present). If $/ has not been changed from default Ruby record separator, then chomp also removes carriage return characters (\n, \r, \r\n).
...
# Take an array or strings and return a single string with elements separated by a single space.
# Iterate through each thing in the array, strip, and join back together 
